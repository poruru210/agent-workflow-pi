...
parent Pi
  ...
  └─ mcp
       └─ lazy MCP discovery (Semble initially)
```
...
`pi-mcp-adapter` exposes MCP through one compact proxy tool and discovers concrete MCP tools on demand. Servers are lazy by default. The initial server is Semble:

```text
uvx --from semble[mcp]==0.5.5 semble
```

Use Semble in the parent for semantic code discovery and include bounded relevant findings in a delegated Job Lease when needed. Use grep for exhaustive literal occurrence searches.
