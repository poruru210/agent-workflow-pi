...
Once activated, `workflow/global-workflow.md` is the normative semantic workflow for objectives, phase ordering, gates, evidence semantics, independent-refutation requirements, correction/convergence, and completion. This file supplies the more specific Pi runtime/responsibility binding: who retains architectural authority and who normally owns candidate-bearing writes. The binding narrows execution ownership without weakening or replacing the semantic/evidence gates in `global-workflow.md`. Higher-priority instructions and more specific applicable project instructions still control scope and authority.
...
- For a non-trivial candidate-bearing implementation, repair, or migration, the normal writer is `workflow-implementer`. Keep tightly coupled surfaces of the same change (for example runtime, public types, focused tests, and user documentation) in one coherent implementer package. Split into multiple writer packages only when ownership and integration boundaries are genuinely independent enough that parallel or separate implementation reduces total cost without creating shared-state conflict.
