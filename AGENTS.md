...
The parent `mcp` proxy provides MCP tools on demand. Prefer Semble semantic search for conceptual or behavioral code discovery in unfamiliar code, then go directly to the returned file/line ranges. Use literal grep when exhaustive exact-occurrence coverage is required. Do not repeat the same search through both methods without a specific reason. When a child needs semantic-search findings, gather them in the parent and include the relevant bounded evidence in its Job Lease.
